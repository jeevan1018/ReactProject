...
const SeriesMatche = () => {
  const { id } = useParams();
    ...
    const Data=seriesData.filter(series=> series?.seriesAdWrapper?.seriesId===seriesId)
    const UserData=MatchesListData && MatchesListData?.typeMatches?.map((serise)=>serise?.seriesMatches?.find((seriseID)=>seriseID?.seriesAdWrapper?.seriesId==id))
